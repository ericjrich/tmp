...
const WATCH50=5,TRIGGER50=6,COLD_HELP=5,COLD_STRONG=7;
...
class RouletteEngine{
 ...
 update(v){v=String(v);if(!ok(v))return false;const d=HIST_DECAY,d2=d*d;this.sumW*=d;this.sumW2*=d2;for(const k of ALL_LABELS)this.histHits[k]*=d;for(const p of POCKETS)this.pocketHits[p]*=d;this.sumW++;this.sumW2++;for(const k of ALL_LABELS)if(BETS.get(k).has(v))this.histHits[k]++;this.pocketHits[v]++;
  for(const def of F50){const st=this.states[def.f],s=def.side(v);if(st.active){st.active.age++;if(s===st.active.t){st.active=null;st.rs=s;st.rl=1}else if(s===st.active.src)st.active.sourceHits++;continue}if(!s){st.rs=null;st.rl=0;continue}if(s===st.rs)st.rl++;else{st.rs=s;st.rl=1}if(st.rl>=TRIGGER50)st.active={src:s,t:def.a[0]===s?def.a[1]:def.a[0],triggerRun:st.rl,sourceHits:st.rl,age:0}}
 ...
 model(){const hist=this.historyNetwork(),ss=F50.map(def=>{const st=this.states[def.f];const watch=!st.active&&st.rs&&st.rl===WATCH50?{src:st.rs,t:def.a[0]===st.rs?def.a[1]:def.a[0],run:st.rl}:null;return{...def,rs:st.rs,rl:st.rl,a:st.active?{...st.active}:null,watch}}),as=ss.filter(x=>x.a),ws=ss.filter(x=>x.watch),cc=T13.map(def=>{const g=this.gaps[def.n],base=coldStrength(g),hn=hist.net.get(def.n)||0,w=base*(1+.15*clamp(hn/2,-1,1));return{...def,g,w,on:g>=COLD_HELP,strong:g>=COLD_STRONG,hist:hn}}),ac=cc.filter(x=>x.on),strongCold=cc.filter(x=>x.strong),activeWeights=new Map();for(const s of as){const hn=hist.net.get(s.a.t)||0;activeWeights.set(s.a.t,1+.12*clamp(hn/2,-1,1))}
...
function ranked13(m){return m.strongCold.map(x=>{const rel=m.rel13.get(x.n)||0;return{n:x.n,label:x.s,g:x.g,score:x.w+.35*rel}}).sort((a,b)=>b.score-a.score||b.g-a.g||a.n.localeCompare(b.n)))}
window.RT={RouletteEngine,POCKETS,BETS,RED,name,cls,ranked50,ranked13,clamp,WATCH50,TRIGGER50,COLD_HELP,COLD_STRONG};
})();